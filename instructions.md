...
- Untracked (intentional): `instructions.md`, `reasonix.toml` (handoff + tool config, never commit)
...
- Git identity (repo-local): `Jakoba <f.jakoba@ma-do.mg>`
...
| `src/contexts/AuthContext.test.tsx` | Session-based auth: session restore + profile by auth_id, login success/failure, logout, `can()` role gating (admin wildcard, pm denial), `firstAllowedPath` fallback order — over mocked `supabase.auth` + `makeApi` |
| `src/components/auth/LoginPage.test.tsx` | Login screen: renders fields, submits credentials to `login()`, surfaces auth errors, redirects when already signed in, shows the email-confirmed banner from `/auth/confirm` |
| `src/components/auth/ConfirmPage.test.tsx` | Email confirmation handler: PKCE `?code=` exchange, `token_hash&type=` verifyOtp (query + hash), signs the fresh session out so `/login` shows "Email confirmed!", already-signed-in short-circuit (detectSessionInUrl, no sign-out), URL error beats stale session, failure paths, no-token error |
...
2. **After 012**: enable Email provider (Supabase → Auth → Providers — on by default), create the first admin (Auth → Add user), deploy the app build.
...
4. **RLS phase 2**: role-scope the remaining tables (leads, opportunities, quotes, purchase_orders, boqs, contracts, survey/installation/integration/atp, inventory, procurement, assets, resources, documents, subcontractors, …).
5. **Deploy**: Vercel/Netlify, set both env vars.
5. Optional follow-ups the user may want (ask before doing):
...
- Git identity: repo-local `Jakoba <f.jakoba@ma-do.mg>`; remote `origin` = HTTPS `https://github.com/kobajah/Manongaerp.git`
