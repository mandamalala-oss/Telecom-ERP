...
- Live-DB probe (2026-08-15): `projects.scope_*`, `sites.means_of_transport` / `transmission_type` / `customer_*`, `boqs.network_type`, `catalog_items`, `quotes/invoices/payments.delivery_type`, and field-op crew/PM columns all exist — i.e. migrations 023–030 appear applied on live. **Migration 031 (tasks.start_date) + 032 (tasks.is_milestone) are NEW — run them on the live DB before/with the Milestone 20/21 build.** **Migration 033 (tasks.parent_id) is NEW for the Milestone 22 XML import.**
...
- Milestone 22 changed: `src/lib/msProjectImport.ts` + `.test.ts`, `src/modules/tasks/MSProjectImportModal.tsx`, `src/modules/tasks/TaskBoard.tsx`, `src/types/index.ts`, `database/schema.sql`, `schema.sql`; new `database/migrations/033_task_parent_id.sql`
...
### Milestone 22 — MS Project XML import (replaces CSV) + task/subtask hierarchy (2026-09-04)

The import format switched from **MS Project CSV to the real MS Project XML file** (`*.xml`, root `<Project xmlns="http://schemas.microsoft.com/project">`), driven by a real French-language sample (`NOKIA PROJECT -1SM-1RFM.xml` at the repo root). The XML schema uses fixed English element names, so **the same parser handles French and English files** with zero per-language logic.
- **`src/lib/msProjectImport.ts` → new `parseMSProjectXML`** (pure, no DOM — regex/scanning over the flat `<Tasks>/<Resource>/<Assignment>` blocks so it runs in the node test env). Language-independent element names; tolerant entity decoding (`&amp;`, `&lt;`, CDATA) for titles/notes; **all date layouts** are pushed through the existing `parseDate` (accepts `yyyy-mm-ddThh:mm:ss`, bare ISO, and `d/m/y`/`m/d/y` — XML itself is always ISO, but hand-edits/other tools aren't).
- **Task/subtask hierarchy preserved**: each summary task + leaf is imported; the level-0 project row (its name equals the project) is skipped; every task carries `parentId` of the nearest enclosing summary (outline-walk by `OutlineLevel`, parents always precede children). Schema gains `tasks.parent_id` (self-FK, `ON DELETE CASCADE`) — **migration `033`**, `Task.parentId`, both schema files synced.
- Predecessor links (`<PredecessorLink><PredecessorUID>`/…`) resolve to dependencies in a second pass (parents/children all included, so links into summary rows survive). Milestones from `<Milestone>1</Milestone>` or a zero-length `<Duration>`.
- Assignees come from `<Assignments>` **restricted to work (person) resources** (type 1; material/cost/equipment rows like FUEL/4X4 RENT are ignored) → first person = `assigneeId`/`assigneeName`, extras appended to `description`.
- **`MSProjectImportModal.tsx`**: instructions/accept now `.xml`; date-locale toggle removed (not needed); preview + unresolved-warnings unchanged. `TaskBoard.importTasks` strips an empty `parentId` like the other blank fields; parents are inserted before children (document order), so the self-FK is always satisfied.
