...
import { generateInvoiceForMilestone, setMilestoneStatusForInvoice } from '@/lib/paymentAutomation'
import { supabase } from '@/lib/supabase'
import type { Quote, Invoice, PurchaseOrder, Payment, Project, ProjectSite, Site } from '@/types'
import type { PaymentSchedule, ProjectPaymentMilestone } from '@/types/v2'

/** Project name with its site codes underneath (a project can span sites). */
function ProjectSiteCell({ name, codes }: { name?: string; codes: string[] }) {
  if (!name && codes.length === 0) return <span className="text-slate-400">—</span>
  return (
    <div className="min-w-0">
      {name && <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{name}</p>}
      {codes.length > 0 && <p className="text-xs font-mono text-slate-400 truncate">{codes.join(', ')}</p>}
    </div>
  )
}
...
type Tab = 'invoices' | 'quotes' | 'purchase_orders' | 'payments' | 'schedules' | 'milestones'
...
export function FinanceModule() {
  ...
  // Milestone billing: schedule templates + the per-project milestone ledger
  // (invoices are auto-generated by the PAC/FAC + advance automation).
  const { data: schedules, error: schErr } = useEntity<PaymentSchedule>(TABLES.paymentSchedules)
  const { data: milestones, refresh: refreshMilestones, error: milErr } = useEntity<ProjectPaymentMilestone>(TABLES.projectPaymentMilestones)
  // Project → sites, so finance can show which site(s) a document covers.
  ...
  // ── Project / site resolution ──────────────────────────────────────────────
  // Finance docs carry a project_id, not a site; the site(s) come from the
  // project_sites junction. A project can cover several sites.
  const projectNameById = useMemo(() => new Map(projects.map(p => [p.id, p.name])), [projects])
  const sitesByProject = useMemo(() => {
    const siteById = new Map(sites.map(s => [s.id, s]))
    const map = new Map<string, Site[]>()
    for (const ps of projectSites) {
      const s = siteById.get(ps.siteId)
      if (!s) continue
  ...
  const projectName = (id?: string) => (id ? projectNameById.get(id) ?? '' : '')
  const siteCodesForProject = (id?: string) => (id ? (sitesByProject.get(id) ?? []).map(s => s.siteId) : [])
  const siteOptions = useMemo(() => [...new Set(sites.map(s => s.siteId))].sort((a, b) => a.localeCompare(b)), [sites])

  // ── Filters (customer / status / site / free-text) ─────────────────────────
  const searchTerm = search.trim().toLowerCase()
  const textMatch = (...vals: (string | undefined)[]) =>
    !searchTerm || vals.some(v => (v ?? '').toLowerCase().includes(searchTerm))
  const siteMatch = (projectId?: string) =>
    !filterSite || siteCodesForProject(projectId).includes(filterSite)

  const visibleInvoices = invoices.filter(i =>
    (!filterCustomer || i.customerName === filterCustomer) &&
    (!filterStatus || i.status === filterStatus) &&
    siteMatch(i.projectId) &&
    textMatch(i.number, i.customerName, projectName(i.projectId), ...siteCodesForProject(i.projectId)))
  const visibleQuotes = quotes.filter(qt =>
    (!filterCustomer || qt.customerName === filterCustomer) &&
    (!filterStatus || qt.status === filterStatus) &&
    siteMatch(qt.projectId) &&
    textMatch(qt.number, qt.customerName, projectName(qt.projectId), ...siteCodesForProject(qt.projectId)))
  const visiblePos = pos.filter(p =>
    (!filterCustomer || p.vendorName === filterCustomer) &&
    (!filterStatus || p.status === filterStatus) &&
    siteMatch(p.projectId) &&
    textMatch(p.number, p.vendorName, projectName(p.projectId), ...siteCodesForProject(p.projectId)))
  const visiblePayments = payments.filter(p => {
    const inv = p.invoiceId ? invoices.find(i => i.id === p.invoiceId) : undefined
    const projectId = inv?.projectId
    return (!filterCustomer || p.customerName === filterCustomer) &&
      siteMatch(projectId) &&
      textMatch(p.invoiceNumber, p.customerName, p.reference, projectName(projectId), ...siteCodesForProject(projectId))
  })
  const visibleMilestones = milestones.filter(m =>
    siteMatch(m.projectId) &&
    textMatch(m.projectName, m.name, m.invoiceNumber, ...siteCodesForProject(m.projectId)))

  // A payment inherits its project/site from the invoice it settles.
  const selPayInvoice = selPay?.invoiceId ? invoices.find(i => i.id === selPay.invoiceId) : undefined
  ...
  const changeStatus = async (inv: Invoice, status: string) => {
    ...
    try {
      ...
      if (goingToPaid) {
        ...
        // A milestone-billed invoice marks its ledger row paid (no-op otherwise).
        await setMilestoneStatusForInvoice(inv.id!, 'paid')
      } else if (leavingPaid) {
        ...
        await setMilestoneStatusForInvoice(inv.id!, 'invoiced')
      } else {
  ...
  // Manually bill a pending milestone (advance/PAC/FAC are auto-billed; this
  // covers 'manual' milestones and retries after a failure).
  ...
  const errorForTab =
    tab === 'invoices' ? invErr :
    tab === 'quotes' ? quoErr :
    tab === 'purchase_orders' ? poErr :
    tab === 'payments' ? payErr :
    tab === 'schedules' ? schErr : milErr
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg flex-wrap">
          {(['invoices','quotes','purchase_orders','payments','schedules','milestones'] as Tab[]).map(t => (
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <input
            ...
            className="text-xs font-semibold rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-2 py-1.5 text-slate-700 dark:text-slate-200 focus:outline-none w-56"
          />
          <select
            value={filterSite}
          ...
          {editable && (tab === 'invoices' || tab === 'quotes' || tab === 'purchase_orders' || tab === 'payments') && (
        ...
        <Card padding={false}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr>
                {['Number','Customer','Project / Site','Subtotal','Tax','Total','Paid','Balance','Issue Date','Due Date','Status',''].map(h => <th key={h} className="th">{h}</th>)}
              </tr></thead>
              <tbody>
                {visibleInvoices.map(inv => (
                  <tr key={inv.id} className="tr-hover cursor-pointer" onClick={() => setSelInv(inv)}>
                    ...
                    <td className="td"><ProjectSiteCell name={projectName(inv.projectId)} codes={siteCodesForProject(inv.projectId)} /></td>
      ...
      {tab === 'quotes' && (
        <Card padding={false}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr>
                {['Number','Customer','Project / Site','Items','Subtotal','Tax','Total','Valid Until','Status',''].map(h => <th key={h} className="th">{h}</th>)}
              </tr></thead>
              <tbody>
                {visibleQuotes.map(q => (
                  <tr key={q.id} className="tr-hover cursor-pointer" onClick={() => setSelQuo(q)}>
                    ...
                    <td className="td"><ProjectSiteCell name={projectName(q.projectId)} codes={siteCodesForProject(q.projectId)} /></td>
      ...
      {tab === 'purchase_orders' && (
        <Card padding={false}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr>
                {['Number','Vendor','Project / Site','Items','Total','Order Date','Expected Delivery','Status',''].map(h => <th key={h} className="th">{h}</th>)}
              </tr></thead>
              <tbody>
                {visiblePos.map(po => (
                  <tr key={po.id} className="tr-hover cursor-pointer" onClick={() => setSelPo(po)}>
                    ...
                    <td className="td"><ProjectSiteCell name={projectName(po.projectId)} codes={siteCodesForProject(po.projectId)} /></td>
            ...
            <Card key={s.id} className="p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {(s.milestones ?? []).map((m, i) => (
                  <span key={m.id ?? i} className="text-xs rounded-full bg-slate-100 dark:bg-slate-700 px-2 py-1 font-semibold text-slate-600 dark:text-slate-300">
                    {m.name} · {m.pct}% · {m.trigger}{m.dueDays ? ` (+${m.dueDays}d)` : ''}
                  </span>
                ))}
              </div>
            </Card>
          ))}
          {schedules.length === 0 && <p className="text-sm text-slate-400">No payment schedules defined.</p>}
        </div>
      )}
      ...
      {tab === 'milestones' && (
        <Card padding={false}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr>
                {['Project','Site','Milestone','Trigger','%','Amount','Due','Status','Invoice',''].map(h => <th key={h} className="th">{h}</th>)}
              </tr></thead>
              <tbody>
                {visibleMilestones.map(m => (
                  <tr key={m.id} className="tr-hover">
                    <td className="td font-semibold">{projectName(m.projectId) || m.projectName || '—'}</td>
                    <td className="td text-xs font-mono text-slate-500">{siteCodesForProject(m.projectId).join(', ') || '—'}</td>
                ...
                {visibleMilestones.length === 0 && (
                  <tr><td colSpan={10} className="td text-center text-sm text-slate-400 py-8">No milestones match — apply a payment schedule to a project or clear the filters.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                { l: 'Project',     v: projectName(selInv.projectId) || '—' },
                { l: 'Site',        v: siteCodesForProject(selInv.projectId).join(', ') || '—' },
              ].map(item => (
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                ...
                { l: 'Project',       v: projectName(selQuo.projectId) || '—' },
                { l: 'Site',          v: siteCodesForProject(selQuo.projectId).join(', ') || '—' },
              ].map(item => (
                <div key={item.l} className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3">
                  <p className="text-xs text-slate-500 font-semibold uppercase tracking-wide">{item.l}</p>
                  <p className="text-sm font-bold text-slate-900 dark:text-white mt-0.5">{item.v}</p>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                ...
                { l: 'Project',            v: projectName(selPo.projectId) || '—' },
                { l: 'Site',               v: siteCodesForProject(selPo.projectId).join(', ') || '—' },
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                ...
                { l: 'Project',       v: projectName(selPayInvoice?.projectId) || '—' },
                { l: 'Site',          v: siteCodesForProject(selPayInvoice?.projectId).join(', ') || '—' },
