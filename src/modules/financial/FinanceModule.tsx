      ...
      {tab === 'milestones' && (
        <Card padding={false}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr>
                {['Project','Milestone','Trigger','%','Amount','Due','Status','Invoice',''].map(h => <th key={h} className="th">{h}</th>)}
              </tr></thead>
              <tbody>
                {milestones.map(m => (
                  <tr key={m.id} className="tr-hover">
                    <td className="td font-semibold">{m.projectName ?? '—'}</td>
                ...
                {milestones.length === 0 && (
                  <tr><td colSpan={9} className="td text-center text-sm text-slate-400 py-8">No milestones yet — apply a payment schedule to a project.</td></tr>
                )}
              </tbody>
            </table>
          </div>
