...
import type { TaskRollup } from '@/lib/taskTimeline'
...
interface KanbanBoardProps {
  ...
  openCreate: () => void
  editable: boolean
  /** Per-task rollups; parents show their rolled-up status and can't be moved. */
 ...
 */
export function KanbanBoard({ tasks, onSelect, onMove, openCreate, editable, rollups }: KanbanBoardProps) {
          ...
          <div key={col.id} className={`flex-shrink-0 w-72 rounded-xl ${col.color} p-3 flex flex-col gap-2`}>
            ...
            <div className="space-y-2 flex-1">
              {colTasks.map((task) => {
                ...
                const isParent = rollups?.get(task.id!)?.isParent ?? false
                return (
                <div key={task.id}
                  ...
                  {isParent && <span className="inline-block text-[10px] font-bold text-slate-500 bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">SUMMARY · auto</span>}
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <div className={`flex items-center gap-1 text-xs ${task.dueDate && isOverdue(task.dueDate) && status !== 'done' ? 'text-red-500' : 'text-slate-400'}`}>
                      {task.dueDate && isOverdue(task.dueDate) && status !== 'done' && <AlertTriangle className="w-3 h-3" />}
                      <Clock className="w-3 h-3" />
                      {task.dueDate}
                    </div>
                  </div>

                  {/* Quick move buttons — parents are rolled up from their
                      subtasks, so they aren't moved by hand. */}
                  {!isParent && (
